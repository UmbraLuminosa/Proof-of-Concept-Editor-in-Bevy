...
impl Plugin for BevyUiPlugin {
    fn build(&self, app: &mut App) {
        app.add_systems(Startup, setup)
            ...
            .add_systems(
                PostUpdate,
                set_ui_viewport.after(super::set_camera_viewport),
            );
    }
}
...
fn set_ui_viewport(
    ...
    mut root_node: Query<&mut Style, With<RootUINode>>,
) {
    ...
    let mut root_style = root_node.single_mut();
    root_style.width = Val::Px(viewport_size.x as f32);
    root_style.height = Val::Px(viewport_size.y as f32);
    root_style.left = Val::Px(viewport_pos.x as f32);
    root_style.top = Val::Px(viewport_pos.y as f32);
}
